angular.module('rsc.address', [
    ...
    .config(function ($stateProvider) {
        ...
        // 地址管理
        $stateProvider.state('rsc.trade_address_manage', {
            url: '/trade_address_manage?type&enter',
            // cache: false,
            views: {
                'center-content': {
                    templateUrl: 'js/module/address/template/address_manage.html',
                    controller: 'trade_address_manage_ctrl as vm'
                }
            }
        })
            // 地址修改 添加
            .state('rsc.trade_address_edit', {
                url: '/trade_address_edit?id?str?source?name?phone',
                // cache: false,
                views: {
                    'center-content': {
                        templateUrl: 'js/module/address/template/address_edit.html',
                        controller: 'trade_address_edit_ctrl as vm'
                    }
                }
            })

            // 地址详情页
            .state('rsc.trade_address_details', {
                url: '/trade_address_details?source&type',
                // cache: false,
                views: {
                    'center-content': {
                        templateUrl: 'js/module/address/template/address_details.html',
                        controller: 'trade_address_details_ctrl as vm'
                    }
                }
            })

            // 联系人添加地址
            .state('rsc.trade_address_contacts', {
                url: '/trade_address_contacts?menu&source?rename?id',
                views: {
                    'center-content': {
                        templateUrl: 'js/module/address/template/address_contacts.html',
                        controller: 'trade_address_contacts_ctrl as vm'
                    }
                }
            })
    })
    .controller('trade_address_manage_ctrl', ['$scope', '$log', 'AddressManageService', 'RscAlert', '$state', '$stateParams', '$ionicModal', '$rootScope',
        function ($scope, $log, AddressManageService, RscAlert, $state, $stateParams, $ionicModal, $rootScope) {
            ...
            vm.editAddress = function (id, str) {
                $state.go("rsc.trade_address_edit", {
                    id: id,
                    str: str,
                    source: 'rsc.trade_address_manage'
                });
    ...
    /**
         * 修改，新增地址详情页sz
         ***/
    .controller('trade_address_edit_ctrl', ['$scope', '$log', 'AddressManageService', 'RscAlert', '$stateParams', '$state', '$cordovaContacts', '$ionicPlatform', 'ionicToast', '$ionicHistory',
     ...
     * 提货 交货 地址列表详情页sz
     * */
    .controller('trade_address_details_ctrl', ['$scope', '$log', 'AddressManageService', 'RscAlert', '$state', '$stateParams', 'Storage', 'ionicToast', '$rootScope', '$ionicHistory', '$ionicModal',
        function ($scope, $log, AddressManageService, RscAlert, $state, $stateParams, Storage, ionicToast, $rootScope, $ionicHistory, $ionicModal) {
            ...
            vm.modal = function () {
                $ionicModal.fromTemplateUrl('./address/template/address_newadd.html', {
                    scope: $scope,
    ...
    .controller('trade_address_contacts_ctrl', ['$scope', '$rootScope', '$log', 'RscAlert', '$state', '$location', '$ionicScrollDelegate', '$stateParams', 'Storage', 'AccountService', 'AddressManageService',
        function ($scope, $rootScope, $log, RscAlert, $state, $location, $ionicScrollDelegate, $stateParams, Storage, AccountService, AddressManageService) {
            ...
            vm.inviteImmediately = function (isSelect) {
                ...
                if (vm.source == 'relation') {
                    $state.go('rsc.replace_colleague', {
                        phone: obj[1],
                        name: obj[0],
