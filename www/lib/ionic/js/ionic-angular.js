  ...
  self.initSwipeBack = function() {
    ...
    function onRelease(ev) {
      if (isPrimary && viewTransition && dragPoints && dragPoints.length > 1) {
        ...
        // private variables because ui-router has no way to src custom data using $state.go
 ...
 *
 * The expression you src in for `on-infinite` is called when the user scrolls
...
function($compile, $ionicConfig, $ionicBind, $ionicViewSwitcher) {
  ...
  return {
    ...
    compile: function(element, attr) {
      ...
      //attributes we src down won't be interpolated yet - we want
      //to src down the 'raw' versions of the attributes
